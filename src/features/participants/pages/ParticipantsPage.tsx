...
export function ParticipantsPage() {
  ...
  return <div className="space-y-7">
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><div className="mb-2 flex items-center gap-2 text-xs font-medium text-ink-400"><span>Ana menü</span><span>/</span><span className="text-ink-600">Katılımcılar</span></div><h1 className="text-2xl font-bold tracking-[-0.035em] text-ink-900 sm:text-[30px]">Katılımcılar</h1><p className="mt-1.5 text-sm text-ink-500">Çalışan listenizi, eğitim yetkilendirmelerini ve gelişim durumunu tek ekrandan yönetin.</p></div><div className="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto"><Button className="h-9 min-w-0 px-3 text-xs sm:px-3.5 sm:text-sm" variant="outline" size="sm" leftIcon={<ArrowDownToLine className="h-4 w-4" />} onClick={exportParticipants}>XLSX indir</Button><Button className="h-9 min-w-0 px-3 text-xs sm:px-3.5 sm:text-sm" variant="outline" size="sm" leftIcon={<Users className="h-4 w-4" />} onClick={openBulkAssign}>Toplu eğitim ata</Button><Button className="h-9 min-w-0 px-3 text-xs sm:px-3.5 sm:text-sm" variant="outline" size="sm" leftIcon={<UserPlus className="h-4 w-4" />} onClick={() => setShowBulkParticipantModal(true)}>Toplu katılımcı</Button><Link to="/dashboard/katilimcilar/yeni" className="inline-flex h-9 min-w-0 items-center justify-center gap-2 rounded-xl bg-brand-600 px-3 text-xs font-semibold text-white shadow-sm shadow-brand-600/20 transition-colors hover:bg-brand-700 sm:h-10 sm:px-4 sm:text-sm"><UserPlus className="h-4 w-4 shrink-0" /> <span className="truncate">Yeni katılımcı</span></Link></div></motion.div>
