...
export function UsersPage() {
  ...
  const { register, handleSubmit, reset, setValue, watch, formState: { errors, isSubmitting } } = userForm
  ...
  function openUserModal() { reset(); setSelectedPermissions([]); setStampDataUrl(''); setStampFileName(''); setIsUserModalOpen(true) }
  ...
  function onSubmit(data: UserForm) { const user: User = { id: Date.now(), firstName: data.firstName, lastName: data.lastName, username: data.username, email: data.email, phone: data.phone || undefined, role: data.role, company: data.company, permissions: data.permissions as ModuleId[], lastLogin: 'Henüz giriş yapmadı', status: 'pending', stampDataUrl: stampDataUrl || undefined, stampFileName: stampFileName || undefined }; setUsers((current) => [user, ...current]); reset(); setSelectedPermissions([]); setStampDataUrl(''); setStampFileName(''); setIsUserModalOpen(false); toast.success('Kullanıcı daveti oluşturuldu', { description: `${user.firstName} ${user.lastName} için davet hazırlandı${user.stampDataUrl ? ' ve kaşesi kaydedildi.' : '.'}` }) }
  ...
  return (
    <div className="space-y-7">
      ...
      {isUserModalOpen && <UserModal register={register} handleSubmit={handleSubmit} onSubmit={onSubmit} errors={errors} isSubmitting={isSubmitting} selectedPermissions={selectedPermissions} selectedRole={selectedUserRole} roleDefinitions={roleDefinitions} stampDataUrl={stampDataUrl} stampFileName={stampFileName} stampInputRef={stampInputRef} onStampChange={handleStampChange} onRemoveStamp={() => { setStampDataUrl(''); setStampFileName('') }} onRoleChange={handleRoleChange} onTogglePermission={toggleUserPermission} onClose={() => setIsUserModalOpen(false)} />}
...
type UserModalProps = { register: ReturnType<typeof useForm<UserForm>>['register']; handleSubmit: ReturnType<typeof useForm<UserForm>>['handleSubmit']; onSubmit: (data: UserForm) => void; errors: ReturnType<typeof useForm<UserForm>>['formState']['errors']; isSubmitting: boolean; selectedPermissions: ModuleId[]; selectedRole: string; roleDefinitions: RoleDefinition[]; stampDataUrl: string; stampFileName: string; stampInputRef: RefObject<HTMLInputElement | null>; onStampChange: (event: ChangeEvent<HTMLInputElement>) => void; onRemoveStamp: () => void; onRoleChange: (event: ChangeEvent<HTMLSelectElement>) => void; onTogglePermission: (permission: ModuleId) => void; onClose: () => void }
function UserModal({ register, handleSubmit, onSubmit, errors, isSubmitting, selectedPermissions, selectedRole, roleDefinitions, stampDataUrl, stampFileName, stampInputRef, onStampChange, onRemoveStamp, onRoleChange, onTogglePermission, onClose }: UserModalProps) {
  const canApproveTraining = selectedRole === 'İSG Uzmanı' || selectedRole === 'İşyeri Hekimi'
  return <ModalShell title="Yeni kullanıcı ekle" subtitle="Kullanıcı hesabı, kaşe ve modül erişimlerini tanımlayın." icon={<UserRound className="h-5 w-5" />} onClose={onClose}><form onSubmit={handleSubmit(onSubmit)} className="space-y-5 px-6 py-6 sm:px-7" noValidate>
    <div className="grid gap-4 sm:grid-cols-2"><Input label="İsim" placeholder="Seda" error={errors.firstName?.message} {...register('firstName')} /><Input label="Soyisim" placeholder="Yalçın" error={errors.lastName?.message} {...register('lastName')} /><Input label="Kullanıcı adı" placeholder="seda.yalcin" error={errors.username?.message} {...register('username')} /><Input label="E-posta" type="email" placeholder="kullanici@ornek.com" icon={<Mail className="h-[18px] w-[18px]" />} error={errors.email?.message} {...register('email')} /><Input label="Telefon" type="tel" placeholder="+90 5xx xxx xx xx" icon={<Phone className="h-[18px] w-[18px]" />} {...register('phone')} /><div><label htmlFor="user-company" className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-ink-500">Firma / kapsam</label><select id="user-company" className="h-12 w-full rounded-xl border border-ink-200 bg-white px-3.5 text-sm outline-none focus:border-brand-500" {...register('company')}><option>Çetka OSGB</option><option>Tüm firmalar</option></select></div></div>
    <div className="grid gap-4 sm:grid-cols-2"><div><label htmlFor="user-role" className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-ink-500">Sistem rolü</label><div className="relative"><select id="user-role" className={cn('h-12 w-full appearance-none rounded-xl border bg-white px-3.5 pr-10 text-sm outline-none focus:border-brand-500', errors.role ? 'border-red-400' : 'border-ink-200')} {...register('role', { onChange: onRoleChange })}><option value="">Rol seçiniz</option>{roleDefinitions.map((role) => <option key={role.id} value={role.name}>{role.name}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-ink-400" /></div>{errors.role && <p className="mt-1.5 text-xs text-red-500">{errors.role.message}</p>}</div><div className="flex items-center rounded-xl bg-brand-50/60 p-3 text-[11px] leading-5 text-brand-800"><KeyRound className="mr-2 h-4 w-4 shrink-0 text-brand-600" />Rol seçildiğinde erişim şablonu uygulanır.</div></div>
    {canApproveTraining && <section className="rounded-2xl border border-brand-200 bg-brand-50/40 p-4"><div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-white text-brand-700 ring-1 ring-brand-100"><Stamp className="h-5 w-5" /></span><div><p className="text-sm font-semibold text-ink-800">Kullanıcı kaşesi</p><p className="mt-1 text-[11px] leading-4 text-ink-500">Bu kişi eğitimi onayladığında sertifikada kendi kaşesi gösterilir.</p></div></div><div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center"><div className="grid h-24 flex-1 place-items-center overflow-hidden rounded-xl border border-dashed border-brand-200 bg-white">{stampDataUrl ? <img src={stampDataUrl} alt="Kullanıcı kaşesi önizlemesi" className="max-h-20 max-w-[90%] object-contain" /> : <div className="text-center text-ink-300"><Stamp className="mx-auto h-5 w-5" /><p className="mt-1 text-[10px]">Şeffaf PNG önerilir</p></div>}</div><div className="flex gap-2 sm:flex-col"><input ref={stampInputRef} type="file" accept="image/jpeg,image/png,image/webp" onChange={onStampChange} className="hidden" /><Button type="button" variant="outline" size="sm" onClick={() => stampInputRef.current?.click()} leftIcon={<Upload className="h-3.5 w-3.5" />}>{stampDataUrl ? 'Kaşeyi değiştir' : 'Kaşe yükle'}</Button>{stampDataUrl && <Button type="button" variant="ghost" size="sm" onClick={onRemoveStamp} leftIcon={<Trash2 className="h-3.5 w-3.5" />}>Kaldır</Button>}</div></div>{stampFileName && <p className="mt-2 truncate text-[10px] font-semibold text-brand-700">{stampFileName}</p>}</section>}
    <PermissionPicker permissions={selectedPermissions} onToggle={onTogglePermission} error={errors.permissions?.message} /><div className="border-t border-ink-100 pt-5"><p className="mb-4 text-xs font-semibold uppercase tracking-[0.12em] text-ink-600">İlk giriş şifresi</p><div className="grid gap-4 sm:grid-cols-2"><Input label="Şifre" type="password" placeholder="En az 6 karakter" error={errors.password?.message} {...register('password')} /><Input label="Şifre tekrar" type="password" placeholder="Şifreyi tekrar girin" error={errors.passwordConfirmation?.message} {...register('passwordConfirmation')} /></div></div><ModalActions onClose={onClose} submitting={isSubmitting} submitLabel="Kullanıcıyı davet et" />
  </form></ModalShell>
}
