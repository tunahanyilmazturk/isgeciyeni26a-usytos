import type { Training, TrainingChapter } from './trainings'
...
/** Ön test — eğitime başlamadan önce bilgi düzeyini ölçer (tüm eğitimlerde ortak) */
 ...
 * Eşleşmeyen konular için generic soru üretici kullanılır.
 */
const topicQuestionPool: Record<string, QuizQuestion[]> = {
  'a) Çalışma mevzuatı ile ilgili bilgiler': [
    ...
    {
      id: 'q-mevzuat-4',
      text: 'Çalışana zarar verme potansiyeli taşıdığı halde hiçbir zarara yol açmadan ucuz atlatılan olaylara ne ad verilir?',
...
/** Konu başlığından generic quiz soruları üretir — havuzda eşleşme olmadığında kullanılır */
function generateGenericQuestions(topic: string, index: number): QuizQuestion[] {
  // Konu başlığından parantezli ön eki ve gereksiz boşlukları temizle
  const cleanTitle = topic.replace(/^[a-zçğıöşüİ0-9]+\)\s*/i, '').replace(/^\d+\.\s*/, '').trim()
  return [
    {
      id: `gen-${index}-1`,
      text: `"${cleanTitle}" konusuyla ilgili aşağıdakilerden hangisi doğrudur?`,
      options: [
        'Bu konu iş sağlığı ve güvenliği açısından önem taşır ve doğru uygulama zorunludur.',
        'Bu konu sadece yöneticileri ilgilendirir, çalışanların bilgisi gerekmez.',
        'Bu konu yalnızca yıllık bir kez hatırlatılması gereken sembolik bir kuraldır.',
        'Bu konu ile ilgili önlem almak tamamen çalışanın inisiyatifindedir.',
      ],
      correctIndex: 0,
    },
    {
      id: `gen-${index}-2`,
      text: `"${cleanTitle}" kapsamında dikkat edilmesi gereken temel prensip aşağıdakilerden hangisidir?`,
      options: [
        'Acil durumlarda hızlı ve panikle hareket etmek',
        'Riskleri önceden tespit edip gerekli önlemleri almak',
        'Kuralları sadece denetim sırasında uygulamak',
        'Kişisel koruyucu donanımı isteğe bağlı kullanmak',
      ],
      correctIndex: 1,
    },
  ]
...
/** Konu metnine göre quiz sorularını getirir — havuzda varsa oradan, yoksa üretir */
function getQuestionsForTopic(topic: string, index: number): QuizQuestion[] {
  return topicQuestionPool[topic] ?? generateGenericQuestions(topic, index)
}

/** Konu metninden quiz başlığı üretir */
function topicToQuizTitle(topic: string): string {
  return topic.replace(/^[a-zçğıöşüİ0-9]+\)\s*/i, '').replace(/^\d+\.\s*/, '').trim()
}

/** Konu için tahmini video süresi üretir */
function estimateDuration(topic: string): string {
  const wordCount = topic.split(' ').length
  const minutes = Math.max(6, Math.min(15, Math.round(wordCount * 1.2)))
  return `${minutes} dk`
}

/** Bir TrainingChapter'ı PreviewChapter'a dönüştürür */
function buildPreviewChapter(chapter: TrainingChapter, globalStepCounter: { n: number }): PreviewChapter {
  const steps: PreviewStep[] = chapter.topics.map((topic, topicIdx) => {
    const stepNumber = globalStepCounter.n++
    const stepId = `${chapter.id}-step-${topicIdx}`
    const questions = getQuestionsForTopic(topic, topicIdx)
    return {
      id: stepId,
      label: `Adım ${stepNumber}`,
      videoTitle: topic,
      videoDuration: estimateDuration(topic),
      quiz: {
        topicId: stepId,
        title: topicToQuizTitle(topic),
        duration: questions.length > 3 ? '5 dk' : '3 dk',
        questions,
      },
    }
  })
...
/** Geriye dönük uyumluluk için — artık kullanılmıyor ama eski importları kırmamak için */
export const trainingPreviewData = buildTrainingPreview({
  id: 'base-low',
  name: 'Az Tehlikeli',
  package: 'Temel Paket',
  risk: 'Az Tehlikeli',
