...
export function CustomersPage() {
  ...
  const handleBulkDelete = () => { setCustomers((current) => current.filter((item) => !selectedIds.includes(item.id))); toast.success(`${selectedIds.length} Firma silindi`); setSelectedIds([]) }
  ...
  function exportCustomers() {
    ...
    toast.success("Firma listesi Excel'e aktarıldı")
  }
  ...
  function openEditCustomer(customer: Customer) {
    ...
    reset({
      ...
      employees: String(customer.employees),
  ...
  function onSubmit(data: CustomerForm) {
    const existing = editingCustomerId === null ? undefined : customers.find((item) => item.id === editingCustomerId)
    const customer: Customer = {
      ...(existing ?? {
        id: Date.now(),
        expertMinutes: 0,
        doctorMinutes: 0,
        contractStatus: 'Teklif aşamasında' as ContractStatus,
        approvalStatus: 'Onay bekliyor' as Customer['approvalStatus'],
    ...
    setCustomers((current) => existing ? current.map((item) => item.id === existing.id ? customer : item) : [customer, ...current])
    reset()
    setEditingCustomerId(null)
    setIsModalOpen(false)
    toast.success(existing ? 'Firma bilgileri güncellendi' : 'Firma başarıyla eklendi', { description: `${customer.name} Firma portföyüne ${existing ? 'kaydedildi' : 'eklendi'}.` })
  }
  ...
  return (
    <div className="space-y-7">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
        <div>
          <div className="mb-2 flex items-center gap-2 text-xs font-medium text-ink-400"><span>Ana menü</span><span>/</span><span className="text-ink-600">Firmalar</span></div>
          <h1 className="text-2xl font-bold tracking-[-0.035em] text-ink-900 sm:text-[30px]">Firmalar</h1>
      ...
      <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: 0.04 }} className="relative overflow-hidden rounded-2xl bg-brand-900 px-5 py-5 text-white shadow-[0_12px_32px_-18px_rgba(18,70,65,0.5)] sm:px-7 sm:py-6">
        ...
        <div className="relative flex flex-col justify-between gap-5 sm:flex-row sm:items-center">
          <div>
            ...
            <h2 className="text-lg font-semibold tracking-[-0.02em] sm:text-xl">Firma portföyünüz kontrol altında.</h2>
      ...
      <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: 0.1 }} className="min-w-0 rounded-2xl border border-ink-200/80 bg-white shadow-[0_4px_18px_-14px_rgba(17,24,39,0.22)]">
        <div className="border-b border-ink-100 p-5 sm:p-6">
          <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
            <div><div className="flex items-center gap-2"><h2 className="text-sm font-semibold text-ink-900">Firma portföyü</h2><span className="rounded-full bg-brand-50 px-2 py-0.5 text-[10px] font-bold text-brand-700">{filteredCustomers.length} kayıt</span></div><p className="mt-1 text-xs text-ink-400">Firma bilgilerine, atamalara ve sözleşme durumuna hızlıca erişin.</p></div>
        ...
        {view === 'card' && (
          <div className="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-3 sm:p-6">
            ...
            {paginatedItems.length === 0 && <div className="col-span-full py-16 text-center"><Search className="mx-auto h-8 w-8 text-ink-300" /><p className="mt-3 text-sm font-semibold text-ink-700">Firma bulunamadı</p></div>}
          </div>
        )}
        <Pagination
