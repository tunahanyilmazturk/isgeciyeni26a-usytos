    ...
    <div className="space-y-7">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
        ...
        <div className="flex flex-wrap gap-2">
          <Link to="/dashboard/firmalar/toplu-isg-import" className="inline-flex h-9 items-center justify-center gap-2 rounded-xl border border-brand-200 bg-brand-50 px-3.5 text-sm font-semibold text-brand-700 transition-colors hover:bg-brand-100"><Upload className="h-4 w-4" /> Toplu aktarım</Link>
          <Button variant="outline" size="sm" leftIcon={<ArrowDownToLine className="h-4 w-4" />} onClick={exportCustomers}>Dışa aktar</Button>
          <Link to="/dashboard/firmalar/yeni" className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-brand-600 px-4 text-sm font-semibold text-white shadow-sm shadow-brand-600/20 transition-colors hover:bg-brand-700"><Plus className="h-4 w-4" /> Yeni müşteri</Link>
        </div>
      </motion.div>
      ...
      <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: 0.1 }} className="min-w-0 rounded-2xl border border-ink-200/80 bg-white shadow-[0_4px_18px_-14px_rgba(17,24,39,0.22)]">
        <div className="border-b border-ink-100 p-5 sm:p-6">
          ...
          {showFilters && <div className="mt-5 grid gap-3 border-t border-ink-100 pt-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
            {[
