...
import { readCustomerById, readCustomers, saveCustomers, type Customer } from '../data/customers'
...
export function CustomerDetailPage() {
  ...
  const companyParticipants = useMemo(
    () => readParticipants().filter((participant) => participant.companyId === company.id),
    [company.id],
  )
  const companyParticipantNames = companyParticipants.map((participant) => participant.name)
  ...
  function handleDelete() {
    ...
    saveCustomers(readCustomers().filter((customer) => customer.id !== company.id))
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: 0.04 }} className="relative overflow-hidden rounded-2xl border border-ink-200/80 bg-white shadow-[0_8px_28px_-18px_rgba(17,24,39,0.28)]">
        ...
        ].map((item) => <button key={item.label} type="button" onClick={() => { if (item.label === 'Giriş listesi') { downloadParticipantLoginList(company.name, companyParticipantNames); toast.success("Giriş listesi Excel'e aktarıldı") } else if (item.label === 'Katılımcı ekle') { navigate(`/dashboard/katilimcilar?company=${encodeURIComponent(company.name)}`) } else action(item.message) }} className="inline-flex min-w-0 items-center justify-center gap-2 rounded-xl border border-ink-200 bg-white px-2.5 py-2 text-xs font-semibold text-ink-600 transition-colors hover:border-brand-200 hover:bg-brand-50 hover:text-brand-700"><item.icon className="h-3.5 w-3.5" /> {item.label}</button>)}</div>
      ...
      {activeTab === 'participants' && <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-ink-200/80 bg-white shadow-[0_4px_18px_-14px_rgba(17,24,39,0.22)]"><div className="flex flex-col justify-between gap-4 border-b border-ink-100 p-5 sm:flex-row sm:items-center sm:p-6"><div><h2 className="text-sm font-semibold text-ink-900">Katılımcılar</h2><p className="mt-1 text-xs text-ink-400">{companyParticipants.length} katılımcı kaydı bu firmaya bağlı.</p></div><Button className="w-full sm:w-auto" size="sm" leftIcon={<Plus className="h-4 w-4" />} onClick={() => action('Katılımcı ekleme ekranı hazırlanacak.')}>Katılımcı ekle</Button></div><div className="relative overflow-x-auto">
        ...
        <table className="w-full min-w-[700px] text-left text-xs"><thead className="border-b border-ink-100 bg-ink-50/40 text-[10px] uppercase tracking-wider text-ink-400"><tr><th className="px-5 py-3.5 font-semibold sm:px-6">Katılımcı</th><th className="px-3 py-3.5 font-semibold">Departman</th><th className="px-3 py-3.5 font-semibold">Eğitim durumu</th><th className="px-3 py-3.5 font-semibold">Son giriş</th><th className="px-5 py-3.5 text-right font-semibold sm:px-6">İşlem</th></tr></thead><tbody className="divide-y divide-ink-100">{companyParticipants.slice(0, 5).map((participant, index) => <tr key={participant.id} className="transition-colors hover:bg-ink-50/50"><td className="px-5 py-4 sm:px-6"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-full bg-brand-50 text-[11px] font-bold text-brand-700">{initials(participant.name)}</span><div><p className="font-semibold text-ink-800">{participant.name}</p><p className="mt-0.5 text-[11px] text-ink-400">{participant.email}</p></div></div></td><td className="px-3 py-4 text-ink-600">{participant.department}</td><td className="px-3 py-4"><span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-semibold text-emerald-700"><CheckCircle2 className="h-3 w-3" /> {participant.trainingStatus === 'successful' ? 'Tamamlandı' : participant.trainingStatus === 'in_progress' ? 'Devam ediyor' : 'Başlamadı'}</span></td><td className="px-3 py-4 text-ink-500">{index + 1} gün önce</td><td className="px-5 py-4 text-right sm:px-6"><button type="button" onClick={() => action(`${participant.name} detayları açılacak.`)} className="rounded-lg p-2 text-ink-400 hover:bg-ink-100 hover:text-ink-700" aria-label="Katılımcı seçenekleri"><MoreHorizontal className="h-4 w-4" /></button></td></tr>)}</tbody></table></div></motion.section>}
