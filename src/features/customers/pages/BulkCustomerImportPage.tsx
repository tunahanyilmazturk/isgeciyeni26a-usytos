...
const requiredColumns = [
  ...
  { column: 'O', label: 'Firma unvanı', detail: 'Firma adı' },
  { column: 'P', label: 'Firma SGK sicil no', detail: 'Eşleştirme anahtarı' },
  { column: 'Q', label: 'Firma ili', detail: 'Konum bilgisi' },
...
const sampleRows = [
  { company: 'Quantis Tekstil', status: 'Yeni Firma', statusTone: 'new', risk: 'Tehlikeli', employees: 84 },
...
export function BulkCustomerImportPage() {
  ...
  function downloadTemplate() {
    ...
    toast.success('Örnek şablon indirildi', { description: 'Hantech ISG Firma aktarım şablonu hazır.' })
  }

  const displayRows = previewRows.length
    ? previewRows.map((row) => ({
        company: row.company,
        status: row.action === 'update' ? 'Güncellenecek' : row.action === 'warning' ? 'Uyarılı' : 'Yeni Firma',
  ...
  return (
    <div className="space-y-7">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
        <Link to="/dashboard/firmalar" className="mb-5 inline-flex items-center gap-2 text-xs font-semibold text-ink-500 transition-colors hover:text-brand-700"><ArrowLeft className="h-4 w-4" /> Firma listesine dön</Link>
        <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><div className="mb-2 flex items-center gap-2 text-xs font-medium text-ink-400"><span>Firmalar</span><ChevronRight className="h-3.5 w-3.5" /><span className="text-ink-600">Toplu aktarım</span></div><h1 className="text-2xl font-bold tracking-[-0.035em] text-ink-900 sm:text-[30px]">Toplu Firma aktarımı</h1><p className="mt-1.5 max-w-2xl text-sm text-ink-500">İSG hizmet sözleşmesi export dosyanızı yükleyin; kayıtları güvenli bir önizleme ile kontrol edip tek seferde aktarın.</p></div><div className="inline-flex w-fit items-center gap-2 rounded-full border border-brand-200 bg-brand-50 px-3 py-1.5 text-xs font-semibold text-brand-700"><FileSpreadsheet className="h-4 w-4" /> XLSX içe aktarma</div></div>
      </motion.div>
