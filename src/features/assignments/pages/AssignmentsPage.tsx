...
export function AssignmentsPage() {
  ...
  const ongoing = assignments.filter((item) => item.status === 'in_progress').length
  const completed = assignments.filter((item) => item.status === 'completed').length
  const overdue = assignments.filter((item) => item.status === 'failed').length
  ...
  return (
    <div className="space-y-7">
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          { label: 'Toplam atama', value: total, detail: 'Tüm eğitim kayıtları', icon: PlayCircle, tone: 'teal' },
          { label: 'Devam eden', value: ongoing, detail: 'Aktif eğitim süreci', icon: TimerReset, tone: 'blue' },
          { label: 'Tamamlanan', value: completed, detail: 'Başarıyla biten', icon: CheckCircle2, tone: 'green' },
          { label: 'Geciken', value: overdue, detail: 'Aksiyon gereken', icon: CircleAlert, tone: 'amber' },
        ].map((stat, index) => (
